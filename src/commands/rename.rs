...
#[command(after_help = "Examples:
  nail rename data.parquet -c \"old=new,foo=bar\" -o renamed.parquet
  nail rename data.csv -c \"First Name=name\" -o -")]
pub struct RenameArgs {
	...
	#[arg(
		short = 'c',
		long = "column",
		help = "Column rename specs (before=after), comma-separated"
	)]
	pub columns: String,
}
...
pub async fn execute(args: RenameArgs) -> NailResult<()> {
	...
	args.common
		.log_if_verbose(&format!("Renaming columns: {:?}", rename_map));
	...
	for target in &target_names {
		if final_column_names.iter().filter(|&n| n == target).count() > 1 {
			return Err(NailError::InvalidArgument(format!(
				"Target column name '{}' conflicts with existing column",
				target
			)));
		}
	}
