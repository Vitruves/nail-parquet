...
#[command(after_help = "Examples:
  nail dedup data.parquet -o unique.parquet
  nail dedup users.csv -s email -o -")]
pub struct DedupArgs {
	...
	#[arg(
		long,
		help = "Keep first occurrence (default) vs last",
		default_value = "first"
	)]
	pub keep: String,
}
...
async fn deduplicate_rows(
	...
	let keep_first = match keep {
		...
		_ => {
			return Err(NailError::InvalidArgument(
				"keep must be 'first' or 'last'".to_string(),
			))
		}
	};
	...
	let partition_by_cols: Vec<String> = dedup_cols.iter().map(|c| format!("\"{}\"", c)).collect();
	...
	let dedup_sql = format!(
		...
		select_columns, partition_by_str, order_by, select_columns
	);
...
async fn deduplicate_columns(df: &DataFrame, keep: &str) -> NailResult<DataFrame> {
	...
	for (i, field_name) in field_names.iter().enumerate() {
		...
		for (j, other_field_name) in field_names.iter().enumerate() {
			if i != j
				&& !processed_columns.contains(other_field_name)
...
mod tests {
	...
	fn create_test_data_with_duplicate_columns() -> (tempfile::TempDir, PathBuf) {
		...
		let dup_array = Float64Array::from(vec![Some(100.0), Some(200.0), Some(300.0)]);
	...
	async fn test_dedup_missing_mode_error() {
		...
		assert!(result
			.unwrap_err()
			.to_string()
			.contains("Must specify either --row-wise or --col-wise"));
	}
	...
	async fn test_dedup_invalid_keep_option() {
		...
		assert!(result
			.unwrap_err()
			.to_string()
			.contains("keep must be 'first' or 'last'"));
	}
