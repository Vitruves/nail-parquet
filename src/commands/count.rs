...
use crate::utils::parquet_utils::{can_use_fast_metadata, get_parquet_row_count_fast};
use clap::Args;
...
pub async fn execute(args: CountArgs) -> NailResult<()> {
	...
	let row_count = if can_use_fast_metadata(&args.common.input) {
		args.common
			.log_if_verbose("Using fast Parquet metadata for counting");
		get_parquet_row_count_fast(&args.common.input).await?
	} else {
		args.common.log_if_verbose("Using DataFusion for counting");
		let df = read_data_with_opts(&args.common.input, args.common.jobs, args.common.batch_size)
			.await?;
		df.clone()
			.count()
			.await
	...
	args.common
		.log_if_verbose(&format!("Counted {} rows", row_count));
	...
	match &args.common.output {
		Some(_output_path) => {
			...
			let result_df = ctx
				.sql(&format!("SELECT {} as row_count", row_count))
				.await
				.map_err(crate::error::NailError::DataFusion)?;
...
mod tests {
	...
	fn create_test_data() -> (tempfile::TempDir, PathBuf) {
		...
		let value_array = Float64Array::from(vec![Some(100.0), Some(200.0), Some(300.0)]);
