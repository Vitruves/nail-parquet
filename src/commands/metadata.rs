...
#[command(after_help = "Examples:
  ...
  nail metadata file.parquet -o metadata.json")]
...
fn collect_basic_metadata(metadata: &ParquetMetaData, items: &mut Vec<(String, String)>) {
	...
	items.push((
		...
		file_metadata.num_rows().to_string(),
	));
	items.push((
	...
	items.push((
		"schema_num_fields".to_string(),
		schema.num_columns().to_string(),
	));

	// Key-value metadata if present
	if let Some(kv_metadata) = file_metadata.key_value_metadata() {
		items.push((
			"key_value_metadata_count".to_string(),
			kv_metadata.len().to_string(),
		));
		for kv in kv_metadata.iter() {
			items.push((
				format!("metadata_{}", kv.key),
...
fn collect_schema_metadata(metadata: &ParquetMetaData, items: &mut Vec<(String, String)>) {
	...
	for (i, field) in schema.columns().iter().enumerate() {
		...
		items.push((
			format!("{}physical_type", col_prefix),
			format!("{:?}", field.physical_type()),
		));
		items.push((
			format!("{}logical_type", col_prefix),
			field
				.logical_type()
				.map_or("None".to_string(), |lt| format!("{:?}", lt)),
		));
		items.push((
			format!("{}max_definition_level", col_prefix),
			field.max_def_level().to_string(),
		));
		items.push((
			format!("{}max_repetition_level", col_prefix),
			field.max_rep_level().to_string(),
		));
		items.push((
			format!("{}is_optional", col_prefix),
			(field.max_def_level() > 0).to_string(),
		));
		items.push((
			format!("{}converted_type", col_prefix),
			format!("{:?}", field.converted_type()),
		));
		items.push((
			format!("{}precision", col_prefix),
			if field.type_precision() >= 0 {
				field.type_precision().to_string()
			} else {
				"N/A".to_string()
			},
		));
		items.push((
			format!("{}scale", col_prefix),
			if field.type_scale() >= 0 {
				field.type_scale().to_string()
			} else {
				"N/A".to_string()
			},
		));
		items.push((
			format!("{}length", col_prefix),
			if field.type_length() >= 0 {
				field.type_length().to_string()
			} else {
				"N/A".to_string()
			},
		));
...
fn collect_row_group_metadata(
	...
	for (i, row_group) in metadata.row_groups().iter().enumerate() {
		...
		items.push((
			format!("{}num_rows", rg_prefix),
			row_group.num_rows().to_string(),
		));
		items.push((
			format!("{}total_byte_size", rg_prefix),
			row_group.total_byte_size().to_string(),
		));
		items.push((
			format!("{}compressed_size", rg_prefix),
			row_group.compressed_size().to_string(),
		));
		items.push((
			format!("{}num_columns", rg_prefix),
			row_group.num_columns().to_string(),
		));
		...
		items.push((
			format!("{}compression_ratio", rg_prefix),
		...
		if detailed {
			...
			for (col_idx, column) in row_group.columns().iter().enumerate() {
				...
				items.push((
					format!("{}file_offset", col_prefix),
					column.file_offset().to_string(),
				));
				items.push((
					format!("{}data_page_offset", col_prefix),
					column.data_page_offset().to_string(),
				));
				items.push((
					format!("{}index_page_offset", col_prefix),
					column
						.index_page_offset()
						.map_or("N/A".to_string(), |o| o.to_string()),
				));
				items.push((
					format!("{}dictionary_page_offset", col_prefix),
					column
...
fn collect_column_chunk_metadata(
	metadata: &ParquetMetaData,
	items: &mut Vec<(String, String)>,
	detailed: bool,
) {
	for (rg_idx, row_group) in metadata.row_groups().iter().enumerate() {
		for (col_idx, column) in row_group.columns().iter().enumerate() {
			...
			items.push((
				format!("{}path", col_prefix),
				column.column_path().to_string(),
			));
			items.push((
				format!("{}type", col_prefix),
				format!("{:?}", column.column_type()),
			));
			items.push((
				format!("{}encodings", col_prefix),
				format!("{:?}", column.encodings()),
			));
			items.push((
				format!("{}compression", col_prefix),
				format!("{:?}", column.compression()),
			));
			items.push((
				format!("{}uncompressed_size", col_prefix),
				column.uncompressed_size().to_string(),
			));
			items.push((
				format!("{}compressed_size", col_prefix),
				column.compressed_size().to_string(),
			));
			items.push((
				format!("{}num_values", col_prefix),
			...
			items.push((
				format!("{}compression_ratio", col_prefix),
				format!("{:.2}%", compression_ratio),
			));
			...
			if detailed {
				...
				items.push((
					format!("{}has_bloom_filter", col_prefix),
					column.bloom_filter_offset().is_some().to_string(),
				));
				items.push((
					format!("{}bloom_filter_offset", col_prefix),
					column
						.bloom_filter_offset()
						.map_or("N/A".to_string(), |o| o.to_string()),
				));
				items.push((
					format!("{}bloom_filter_length", col_prefix),
					column
						.bloom_filter_length()
			...
			if let Some(stats) = column.statistics() {
				items.push((
					format!("{}has_min_max", col_prefix),
					(stats.min_bytes_opt().is_some() && stats.max_bytes_opt().is_some())
						.to_string(),
				));
				items.push((
					format!("{}null_count", col_prefix),
					stats
						.null_count_opt()
						.map_or("N/A".to_string(), |c| c.to_string()),
				));
				items.push((
					format!("{}distinct_count", col_prefix),
					stats
						.distinct_count_opt()
						.map_or("N/A".to_string(), |c| c.to_string()),
				));
				...
				{
					items.push((
						format!("{}min_value_bytes", col_prefix),
						format!("{:?}", min_bytes),
					));
					items.push((
						format!("{}max_value_bytes", col_prefix),
						format!("{:?}", max_bytes),
					));
				}
			} else {
...
fn collect_compression_metadata(metadata: &ParquetMetaData, items: &mut Vec<(String, String)>) {
	...
	for row_group in metadata.row_groups().iter() {
		for column in row_group.columns().iter() {
			...
			let entry = compression_stats
				.entry(compression.clone())
				.or_insert((0i64, 0i64, 0usize));
	...
	for (compression, (uncompressed, compressed, count)) in compression_stats {
		...
		items.push((
			format!("{}uncompressed_size", comp_prefix),
			uncompressed.to_string(),
		));
		items.push((
			format!("{}compressed_size", comp_prefix),
			compressed.to_string(),
		));
		items.push((format!("{}chunk_count", comp_prefix), count.to_string()));
		items.push((
			format!("{}ratio", comp_prefix),
			if uncompressed > 0 {
	...
	items.push((
		"overall_compression_ratio".to_string(),
...
fn collect_encoding_metadata(metadata: &ParquetMetaData, items: &mut Vec<(String, String)>) {
	...
	for (encoding, count) in encoding_stats {
		...
		items.push((
			format!("{}usage_percentage", enc_prefix),
			format!("{:.2}%", (count as f64 / total_chunks as f64) * 100.0),
		));
	}
}

fn collect_statistics_metadata(
	metadata: &ParquetMetaData,
	items: &mut Vec<(String, String)>,
	detailed: bool,
) {
		...
		for (col_idx, column) in row_group.columns().iter().enumerate() {
			...
			if let Some(stats) = column.statistics() {
				...
				if detailed {
					...
					items.push((
						format!("{}has_min_max", stat_prefix),
						(stats.min_bytes_opt().is_some() && stats.max_bytes_opt().is_some())
							.to_string(),
					));
					items.push((
						format!("{}null_count", stat_prefix),
						stats
							.null_count_opt()
							.map_or("N/A".to_string(), |c| c.to_string()),
					));
					items.push((
						format!("{}distinct_count", stat_prefix),
						stats
							.distinct_count_opt()
							.map_or("N/A".to_string(), |c| c.to_string()),
					));
	...
	items.push(("total_column_chunks".to_string(), total_chunks.to_string()));
	items.push((
		"chunks_with_statistics".to_string(),
		total_chunks_with_stats.to_string(),
	));
	items.push((
		"chunks_with_null_count".to_string(),
		chunks_with_null_count.to_string(),
	));
	items.push((
		"chunks_with_distinct_count".to_string(),
		chunks_with_distinct_count.to_string(),
	));
	items.push((
		"chunks_with_min_max".to_string(),
		chunks_with_min_max.to_string(),
	));
	items.push(("total_null_count".to_string(), total_null_count.to_string()));
	items.push((
		"total_distinct_count".to_string(),
		total_distinct_count.to_string(),
	));
	items.push((
		"statistics_coverage".to_string(),
		format!(
			"{:.2}%",
			(total_chunks_with_stats as f64 / total_chunks as f64) * 100.0
		),
	));
}
...
async fn create_metadata_dataframe(items: Vec<(String, String)>) -> NailResult<DataFrame> {
	...
	if items.is_empty() {
		return ctx
			.sql("SELECT '' as key, '' as value WHERE 1=0")
			.await
...
mod tests {
	...
	async fn test_metadata_non_parquet_file() {
		...
		assert!(result
			.unwrap_err()
			.to_string()
			.contains("Metadata command only supports Parquet files"));
	}
}
