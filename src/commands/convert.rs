...
mod tests {
	...
	async fn create_test_dataframe() -> NailResult<DataFrame> {
		...
		let value_array = Arc::new(datafusion::arrow::array::Float64Array::from(vec![
			10.5, 20.0, 30.5, 40.0, 50.5,
		]));

		let batch = RecordBatch::try_new(schema.clone(), vec![id_array, name_array, value_array])?;
	...
	async fn test_execute_csv_to_parquet() {
		...
		let df = create_test_dataframe().await.unwrap();
		crate::utils::io::write_data(&df, &input_path, Some(&crate::utils::FileFormat::Csv))
			.await
			.unwrap();
	...
	async fn test_execute_with_empty_dataframe() {
		...
		crate::utils::io::write_data(&empty_df, &input_path, None)
			.await
			.unwrap();
