...
#[command(after_help = "Examples:
  nail outliers data.parquet -c value --method iqr
  nail outliers metrics.csv -c rate --method zscore -o flagged.csv")]
pub struct OutliersArgs {
	...
	#[arg(
		short,
		long,
		help = "Comma-separated column names or regex patterns for outlier detection"
	)]
...
pub async fn execute(args: OutliersArgs) -> NailResult<()> {
	...
	if args.remove {
		...
		let cleaned_df = match args.method {
			OutlierMethod::Iqr => {
				remove_outliers_iqr(&df, &target_columns, args.iqr_multiplier).await?
			}
			OutlierMethod::ZScore => {
				remove_outliers_zscore(&df, &target_columns, args.z_score_threshold).await?
			}
			OutlierMethod::ModifiedZScore => {
				remove_outliers_modified_zscore(&df, &target_columns, args.z_score_threshold)
					.await?
			}
			OutlierMethod::IsolationForest => {
				remove_outliers_isolation_forest(&df, &target_columns).await?
			}
		};
		...
		output_handler
			.handle_output(&cleaned_df, "cleaned_data")
			.await?;
	} else {
		args.common.log_if_verbose(&format!(
...
async fn detect_outliers_iqr(
	...
	for (idx, column) in columns.iter().enumerate() {
		...
		let q1_value =
			if let Some(q1_array) = batch.column(0).as_any().downcast_ref::<Float64Array>() {
		...
		let q3_value =
			if let Some(q3_array) = batch.column(1).as_any().downcast_ref::<Float64Array>() {
...
async fn detect_outliers_zscore(
	...
	for (idx, column) in columns.iter().enumerate() {
		...
		let mean_val =
			if let Some(mean_array) = batch.column(0).as_any().downcast_ref::<Float64Array>() {
		...
		let stddev_val =
			if let Some(stddev_array) = batch.column(1).as_any().downcast_ref::<Float64Array>() {
...
async fn detect_outliers_modified_zscore(
	...
	for (idx, column) in columns.iter().enumerate() {
		...
		let median_val =
			if let Some(median_array) = batch.column(0).as_any().downcast_ref::<Float64Array>() {
		...
		let mad_val =
			if let Some(mad_array) = mad_batch.column(0).as_any().downcast_ref::<Float64Array>() {
...
async fn detect_outliers_isolation_forest(
	...
	for (idx, column) in columns.iter().enumerate() {
		...
		let min_val =
			if let Some(min_array) = batch.column(2).as_any().downcast_ref::<Float64Array>() {
		...
		let max_val =
			if let Some(max_array) = batch.column(3).as_any().downcast_ref::<Float64Array>() {
...
async fn remove_outliers_zscore(
	...
	for column in columns.iter() {
		...
		where_conditions.push(format!(
			"ABS(({} - {}) / {}) <= {}",
			column, mean_val, stddev_val, threshold
		));
	}
...
async fn remove_outliers_modified_zscore(
	...
	for column in columns.iter() {
		...
		where_conditions.push(format!(
			"ABS(0.6745 * ({} - {}) / {}) <= {}",
			column, median_val, mad_val, threshold
		));
	}
...
mod tests {
	...
	async fn test_outliers_no_numeric_columns() {
		...
		assert!(result
			.unwrap_err()
			.to_string()
			.contains("No numeric columns found"));
	}
