...
#[command(after_help = "Examples:
  nail binning data.parquet -c age --bins 10 -o binned.parquet
  nail binning sales.csv -c price --method equal-width -o -")]
pub struct BinningArgs {
	...
	#[arg(
		...
		long,
		default_value = "10",
		help = "Number of bins or custom edges (e.g., \"5\" or \"0,10,50,100\")"
	)]
	pub bins: String,
...
pub async fn execute(args: BinningArgs) -> NailResult<()> {
	...
	let labels = args.labels.as_ref().map(|labels_str| {
		labels_str
			.split(',')
	...
	args.common
		.log_if_verbose(&format!("Binning columns: {:?}", columns));
	args.common
		.log_if_verbose(&format!("Method: {:?}", args.method));
	if let Some(edges) = &bin_edges {
		args.common
			.log_if_verbose(&format!("Bin edges: {:?}", edges));
	} else {
		args.common
			.log_if_verbose(&format!("Number of bins: {}", n_bins.unwrap_or(10)));
	}
	...
	for column_name in &columns {
		args.common
			.log_if_verbose(&format!("Processing column: {}", column_name));

		// Calculate statistics
		let agg_df = result_df.clone().aggregate(
			vec![],
			vec![
		...
		args.common
			.log_if_verbose(&format!("Using bin edges for {}: {:?}", column_name, edges));
		...
		for i in 0..edges.len() - 1 {
			...
			let condition = if i == edges.len() - 2 {
				...
				col(*column_name)
					.gt_eq(lit(lower))
					.and(col(*column_name).lt_eq(lit(upper)))
			} else {
				col(*column_name)
					.gt_eq(lit(lower))
					.and(col(*column_name).lt(lit(upper)))
			};
...
fn parse_bins(
	bins_str: &str,
	_method: &BinningMethod,
) -> NailResult<(Option<Vec<f64>>, Option<usize>)> {
...
fn calculate_equal_width_edges(
	min_val: f64,
	max_val: f64,
	n_bins: usize,
	include_lowest: bool,
...
async fn calculate_equal_frequency_edges(
	df: &DataFrame,
	column_name: &str,
	n_bins: usize,
) -> NailResult<Vec<f64>> {
	// Get sorted values to calculate quantiles
	let sorted_df = df.clone().sort(vec![col(column_name).sort(true, true)])?;
	...
	for batch in batches {
		let array = batch.column_by_name(column_name).ok_or_else(|| {
			NailError::InvalidArgument(format!("Column '{}' not found", column_name))
		})?;
		...
		if let Some(float_array) = array.as_any().downcast_ref::<Float64Array>() {
			...
			return Err(NailError::InvalidArgument(format!(
				"Column '{}' must be numeric for equal-frequency binning",
				column_name
			)));
		}
	}
...
fn extract_float_value(
	...
	if let Some(float_array) = array.as_any().downcast_ref::<Float64Array>() {
		if float_array.is_null(row_idx) {
			return Err(NailError::InvalidArgument(
				"Null value encountered".to_string(),
			));
		}
		...
		Err(NailError::InvalidArgument(format!(
			"Unexpected data type for numeric value: {:?}",
			array.data_type()
		)))
	}
}
...
mod tests {
	...
	async fn create_test_dataframe() -> NailResult<DataFrame> {
		...
		let value_array = Arc::new(Float64Array::from(vec![
			1.5, 5.0, 15.0, 25.0, 35.0, 45.0, 55.0, 65.0, 75.0, 85.0,
		]));
		let category_array = Arc::new(StringArray::from(vec![
			"A", "B", "A", "B", "A", "B", "A", "B", "A", "B",
		]));

		let batch = RecordBatch::try_new(schema.clone(), vec![value_array, category_array])?;
	...
	fn test_extract_float_value() {
		...
		let batch = RecordBatch::try_new(schema.clone(), vec![float_array, int_array]).unwrap();
	...
	async fn test_execute_equal_width_binning() {
		...
		crate::utils::io::write_data(&df, &input_path, None)
			.await
			.unwrap();
