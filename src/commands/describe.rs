...
pub async fn execute(args: DescribeArgs) -> NailResult<()> {
	...
	print_field("Size", &format!("{:.2} MB ({} bytes)", file_size_mb, file_size));
	...
	print_metric("Estimated Memory", &format!("{:.2} MB", estimated_memory_mb), "yellow");
	...
	println!();
	print_metric("Data Density", &format!("{:.2}%", density), if density > 95.0 { "green" } else if density > 80.0 { "yellow" } else { "red" });
	print_metric("Non-Null Values", &non_null_cells.to_string(), "white");
	print_metric("Null Values", &null_info.total_nulls.to_string(), if null_info.total_nulls == 0 { "green" } else { "yellow" });
	print_metric("Null Percentage", &format!("{:.2}%", null_info.null_percentage), if null_info.null_percentage < 1.0 { "green" } else if null_info.null_percentage < 10.0 { "yellow" } else { "red" });

	if null_info.columns_with_nulls > 0 {
		print_metric("Columns w/ Nulls", &format!("{} of {}", null_info.columns_with_nulls, total_columns), "yellow");
	}

	if duplicate_info.estimated_duplicates > 0 {
		print_metric("Est. Duplicate Rows", &duplicate_info.estimated_duplicates.to_string(), "yellow");
		print_metric("Duplicate %", &format!("{:.2}%", duplicate_info.duplicate_percentage), "yellow");
	}
	...
	if !numeric_cols.is_empty() {
		...
		println!("  {}", display_cols.iter().map(|c| c.to_string()).collect::<Vec<_>>().join(", "));
		if numeric_cols.len() > 10 {
			println!("  {} ... and {} more", "".dimmed(), (numeric_cols.len() - 10).to_string().dimmed());
		}
	}
	...
	if !string_cols.is_empty() {
		...
		if string_cols.len() > 10 {
			println!("  {} ... and {} more", "".dimmed(), (string_cols.len() - 10).to_string().dimmed());
		}
	}
	...
	if type_counts.len() > 4 {
		...
		for (dtype, count) in sorted_types {
			println!("  {:30} {}", dtype.bright_white(), count.to_string().yellow());
		}
	}
...
async fn estimate_duplicates(df: &DataFrame, total_rows: usize) -> NailResult<DuplicateInfo> {
	...
	let sql = format!("SELECT COUNT(*) as cnt FROM (SELECT DISTINCT {} FROM temp_table)", select_cols);

	let result = ctx.sql(&sql).await?;
	let batches = result.collect().await?;

	let distinct_count = if let Some(batch) = batches.first() {
		if let Some(int_array) = batch.column(0).as_any().downcast_ref::<datafusion::arrow::array::Int64Array>() {
