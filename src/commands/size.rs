...
#[command(after_help = "Examples:
  ...
  nail size file.parquet -o size.json")]
pub struct SizeArgs {
	#[command(flatten)]
	...
	match &args.common.output {
		Some(_output_path) => {
			...
			let mut size_data = vec![
				...
				format!(
					"'File size' as metric, '{}' as value",
					if args.bits {
						format!("{} bits", file_size * 8)
					} else {
			...
			if args.rows && row_count > 0 {
				...
				size_data.push(format!(
					"'Average per row' as metric, '{}' as value",
					avg_per_row
				));
			}
			...
			size_data.push(format!(
				"'Estimated uncompressed memory usage' as metric, '{}' as value",
				if args.bits {
					format!("{} bits", total_memory * 8)
				} else {
			...
			let sql = format!(
				...
				if size_data.len() > 1 {
					format!(
						" UNION ALL SELECT {}",
						size_data[1..].join(" UNION ALL SELECT ")
					)
				} else {
					String::new()
				}
		...
		None => {
			...
			println!(
				"Estimated uncompressed memory usage: {}",
				if args.bits {
					format!("{} bits", total_memory * 8)
				} else {
					human_bytes(total_memory)
...
fn estimate_column_memory_size(
	...
	match data_type {
		...
		DataType::LargeBinary => row_count * (50 + 8),
