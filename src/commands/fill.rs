...
#[command(after_help = "Examples:
  nail fill data.parquet --method mean -c price -o filled.parquet
  nail fill timeseries.csv --method ffill -c value -o -")]
pub struct FillArgs {
	#[command(flatten)]
...
pub async fn execute(args: FillArgs) -> NailResult<()> {
	...
	args.common.log_if_verbose(&format!(
		"Filling missing values in {} columns using {:?} method",
		columns.len(),
		args.method
	));

	let result_df = fill_missing_values(
		&df,
		&columns,
		&args.method,
		args.value.as_deref(),
...
async fn fill_missing_values(
	...
	for field in schema.fields() {
		...
		if columns.contains(field_name) {
			let filled_expr = match method {
				FillMethod::Value => {
					...
					match field.data_type() {
						datafusion::arrow::datatypes::DataType::Int64 => {
							let val: i64 = fill_val.parse().map_err(|_| {
						...
						datafusion::arrow::datatypes::DataType::Float64 => {
							let val: f64 = fill_val.parse().map_err(|_| {
								NailError::InvalidArgument(format!(
									"Invalid float value: {}",
									fill_val
								))
						...
						datafusion::arrow::datatypes::DataType::Utf8 => coalesce(vec![
							Expr::Column(datafusion::common::Column::new(
								None::<String>,
								field_name,
							)),
						...
						_ => Expr::Column(datafusion::common::Column::new(
							None::<String>,
							field_name,
						)),
					}
				...
				FillMethod::Mean => {
					...
					match field.data_type() {
						...
						_ => {
							return Err(NailError::Statistics(format!(
								"Mean calculation not supported for column '{}' of type {:?}",
								field_name,
								field.data_type()
							)));
						}
				...
				FillMethod::Median => {
					match field.data_type() {
						...
						_ => {
							return Err(NailError::Statistics(format!(
								"Median calculation not supported for column '{}' of type {:?}",
								field_name,
								field.data_type()
							)));
						}
			...
			select_exprs.push(Expr::Column(datafusion::common::Column::new(
				None::<String>,
				field_name,
			)));
		}
	...
	if matches!(
		method,
		FillMethod::Mean | FillMethod::Median | FillMethod::Mode
	) {
		...
		for col_name in columns {
			...
			match method {
				FillMethod::Mean => match field.data_type() {
					datafusion::arrow::datatypes::DataType::Float64
					| datafusion::arrow::datatypes::DataType::Int64 => {
						let mean_sql = format!(
							"SELECT AVG(\"{}\") as stat_value FROM {}",
							col_name, table_name
						);
				...
				FillMethod::Median => match field.data_type() {
					datafusion::arrow::datatypes::DataType::Float64
					| datafusion::arrow::datatypes::DataType::Int64 => {
						let median_sql = format!(
							"SELECT APPROX_PERCENTILE_CONT(\"{}\", 0.5) as stat_value FROM {}",
							col_name, table_name
						);
				...
				let filled_expr = match method {
					FillMethod::Mean => {
						if let Some(&stat_val) = stats_values.get(field_name) {
							match field.data_type() {
								datafusion::arrow::datatypes::DataType::Float64 => coalesce(vec![
									Expr::Column(datafusion::common::Column::new(
										None::<String>,
										field_name,
									)),
								...
								datafusion::arrow::datatypes::DataType::Int64 => coalesce(vec![
									Expr::Column(datafusion::common::Column::new(
										None::<String>,
										field_name,
									)),
...
async fn fill_forward_backward(
	df: &DataFrame,
	columns: &[String],
	method: &FillMethod,
) -> NailResult<DataFrame> {
...
fn forward_fill_array(
	array: datafusion::arrow::array::ArrayRef,
) -> NailResult<datafusion::arrow::array::ArrayRef> {
