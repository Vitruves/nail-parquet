...
#[command(after_help = "Examples:
  nail sort data.parquet -c price -o sorted.parquet
  nail sort sales.csv -c date --desc -o -")]
pub struct SortArgs {
	#[command(flatten)]
...
impl SortStrategy {
	fn from_str(s: &str) -> NailResult<Self> {
		match s.to_lowercase().as_str() {
			...
			_ => Err(NailError::InvalidArgument(format!(
...
pub async fn execute(args: SortArgs) -> NailResult<()> {
	...
	let df = if matches!(null_handling, NullHandling::Skip) {
		args.common
			.log_if_verbose("Filtering out rows with null values in sort columns");
	...
	args.common
		.log_if_verbose(&format!("Sorting by columns: {:?}", sort_columns));
	...
	let strategies = if let Some(strat_list) = &args.strategy {
		if strat_list.len() != sort_columns.len() && strat_list.len() != 1 {
			return Err(NailError::InvalidArgument(format!(
				"Number of strategies ({}) must match number of columns ({}) or be 1 for all",
				strat_list.len(),
				sort_columns.len()
			)));
		}
	...
	let descending_flags = if let Some(desc_list) = &args.descending {
		if desc_list.len() != sort_columns.len() && desc_list.len() != 1 {
			return Err(NailError::InvalidArgument(format!(
				"Number of descending flags ({}) must match number of columns ({}) or be 1 for all",
				desc_list.len(),
				sort_columns.len()
			)));
		}
	...
	args.common
		.log_if_verbose(&format!("Applying {} sort expressions", sort_exprs.len()));
...
fn create_sort_expression(
	...
	let expr = match strategy {
		...
		SortStrategy::Alphabetic => match data_type {
			DataType::Utf8 | DataType::LargeUtf8 => {
				if case_insensitive {
					lower(base_col)
				} else {
...
mod tests {
	...
	fn test_sort_strategy_from_str() {
		assert!(matches!(
			SortStrategy::from_str("numeric").unwrap(),
			SortStrategy::Numeric
		));
		assert!(matches!(
			SortStrategy::from_str("date").unwrap(),
			SortStrategy::Date
		));
		assert!(matches!(
			SortStrategy::from_str("alphabetic").unwrap(),
			SortStrategy::Alphabetic
		));
		assert!(matches!(
			SortStrategy::from_str("alpha-num").unwrap(),
			SortStrategy::AlphabeticNumeric
		));
		assert!(matches!(
			SortStrategy::from_str("num-alpha").unwrap(),
			SortStrategy::NumericAlphabetic
		));
		assert!(matches!(
			SortStrategy::from_str("hour").unwrap(),
			SortStrategy::Hour
		));
		assert!(matches!(
			SortStrategy::from_str("auto").unwrap(),
			SortStrategy::Auto
		));
	...
	fn test_null_handling_from_str() {
		assert!(matches!(
			NullHandling::from_str("first").unwrap(),
			NullHandling::First
		));
		assert!(matches!(
			NullHandling::from_str("last").unwrap(),
			NullHandling::Last
		));
		assert!(matches!(
			NullHandling::from_str("skip").unwrap(),
			NullHandling::Skip
		));
