...
#[command(after_help = "Examples:
  nail shuffle data.parquet -o shuffled.parquet
  nail shuffle data.csv --random 42 -o -")]
...
pub async fn execute(args: ShuffleArgs) -> NailResult<()> {
	...
	if args.common.verbose {
		...
		args.common
			.log_if_verbose(&format!("Shuffling {} rows", total_rows));
	}
	...
	output_handler
		.handle_output(&shuffled_df, "shuffle")
		.await?;
...
async fn shuffle_dataframe(
	...
	if let Some(seed_val) = seed {
				...
				match field.data_type() {
					...
					_ => {
						...
						columns.push(std::sync::Arc::new(
							datafusion::arrow::compute::concat(&arrays).unwrap(),
						));
					}
				}
			}
		...
		let result = ctx
			.sql("SELECT * FROM temp_table ORDER BY RANDOM()")
			.await?;
		Ok(result)
	}
}
