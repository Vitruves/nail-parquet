...
#[command(after_help = "Examples:
  nail tail data.parquet -n 10
  cat data.csv | nail tail -")]
pub struct TailArgs {
	#[command(flatten)]
...
async fn execute_parquet_optimized(args: TailArgs) -> NailResult<()> {
	...
	args.common
		.log_if_verbose(&format!("Total rows (from metadata): {}", total_rows));
	...
	if total_rows <= args.number {
		...
		let df = ctx
			.read_parquet(
				args.common.input.to_str().unwrap(),
		...
		let tail_df = df
			.limit(skip_rows, Some(args.number))
			.map_err(crate::error::NailError::DataFusion)?;

		args.common.log_if_verbose(&format!(
			"Displaying last {} rows (total: {})",
