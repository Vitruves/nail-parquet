...
use crate::utils::stats::{calculate_correlations, select_columns_by_pattern, CorrelationType};
use clap::Args;
use clap::ValueEnum;
...
#[command(after_help = "Examples:
  nail correlations data.parquet -c price,quantity,rating
  nail correlations stats.csv --method spearman -o -")]
pub struct CorrelationsArgs {
	...
	#[arg(
		long,
		help = "Number of decimal places for correlation values",
		default_value = "4"
	)]
	pub digits: usize,
}
...
pub async fn execute(args: CorrelationsArgs) -> NailResult<()> {
	...
	let target_columns = if let Some(col_spec) = &args.columns {
		...
		if !non_numeric_columns.is_empty() {
			let non_numeric_info: Vec<String> = non_numeric_columns
				.iter()
		...
		if numeric_columns.len() < 2 {
			return Err(NailError::Statistics(format!(
				"Need at least 2 numeric columns for correlation. Found {} numeric columns: {:?}",
				numeric_columns.len(),
				numeric_columns
			)));
		}
	...
	args.common.log_if_verbose(&format!(
		"Computing {:?} correlations for {} numeric columns: {:?}",
		args.correlation_type,
		target_columns.len(),
		target_columns
	));
	args.common.log_if_verbose(&format!(
		"Using {} decimal places for correlation values",
		args.digits
	));
	...
	output_handler
		.handle_output(&corr_df, "correlations")
		.await?;
...
mod tests {
	...
	async fn create_test_dataframe() -> NailResult<DataFrame> {
		...
		let batch =
			RecordBatch::try_new(schema.clone(), vec![x_array, y_array, z_array, cat_array])?;
	...
	async fn create_minimal_dataframe() -> NailResult<DataFrame> {
		...
		let batch = RecordBatch::try_new(schema.clone(), vec![a_array, b_array])?;
	...
	async fn test_execute_with_non_numeric_columns() {
		...
		assert!(result
			.unwrap_err()
			.to_string()
			.contains("Non-numeric columns found"));
	}
	...
	async fn test_execute_with_insufficient_columns() {
		...
		assert!(result
			.unwrap_err()
			.to_string()
			.contains("Need at least 2 numeric columns"));
	}
