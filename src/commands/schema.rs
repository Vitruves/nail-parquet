...
pub async fn execute(args: SchemaArgs) -> NailResult<()> {
	...
	let schema_info: Vec<SchemaField> = schema
		.fields()
		.iter()
	...
	args.common
		.log_if_verbose(&format!("Schema contains {} fields", schema_info.len()));
	...
	if let Some(output_path) = &args.common.output {
		if matches!(args.common.format, Some(crate::cli::OutputFormat::Json)) {
			...
			let json_content = serde_json::to_string_pretty(&schema_info).map_err(|e| {
				crate::error::NailError::InvalidArgument(format!("JSON serialization error: {}", e))
			})?;
			std::fs::write(output_path, json_content)?;
			args.common.log_if_verbose(&format!(
				"Schema JSON written to: {}",
				output_path.display()
			));
			return Ok(());
		}
	...
	let schema_sql = schema_info
		.iter()
		.map(|field| {
			format!(
				"'{}' as name, '{}' as data_type, {} as nullable",
				field.name.replace("'", "''"),
	...
	let result_df = if schema_info.is_empty() {
		ctx.sql("SELECT '' as name, '' as data_type, false as nullable WHERE 1=0")
			.await
			.map_err(crate::error::NailError::DataFusion)?
	} else {
		ctx.sql(&format!("SELECT {}", schema_sql))
			.await
			.map_err(crate::error::NailError::DataFusion)?
	};
