...
#[command(after_help = "Examples:
  nail optimize data.parquet -o smaller.parquet
  nail optimize file.parquet --compression zstd -o out.parquet")]
pub struct OptimizeArgs {
	...
	#[arg(
		long,
		help = "Sort by columns for better compression (comma-separated)"
	)]
...
pub async fn execute(args: OptimizeArgs) -> NailResult<()> {
	...
	args.common
		.log_if_verbose(&format!("Input file contains {} rows", count));
	...
	let sorted_df = if let Some(sort_cols) = &args.sort_by {
		...
		args.common
			.log_if_verbose(&format!("Sorting data by columns: {:?}", columns));

		// Create sort expressions
		let sort_exprs: Vec<datafusion::logical_expr::SortExpr> = columns
			.iter()
	...
	let output_path = args.common.output.clone().unwrap_or_else(|| {
		...
		args.common
			.input
			.with_file_name(format!("{}_optimized.parquet", stem))
	});

	args.common.log_if_verbose(&format!(
		"Writing optimized file to: {}",
		output_path.display()
	));
	...
	if args.common.verbose {
		if let Ok(original_size) = std::fs::metadata(&args.common.input).map(|m| m.len()) {
			if let Ok(optimized_size) = std::fs::metadata(&output_path).map(|m| m.len()) {
				...
				args.common
					.log_if_verbose(&format!("Original size: {} bytes", original_size));
				args.common
					.log_if_verbose(&format!("Optimized size: {} bytes", optimized_size));
				args.common
					.log_if_verbose(&format!("Size reduction: {:.1}%", reduction));
			}
		}
	}
	...
	if args.validate {
		...
		if original_count != validated_count {
			return Err(NailError::InvalidArgument(format!(
				"Validation failed: row count mismatch (original: {}, optimized: {})",
				original_count, validated_count
			)));
		}

		args.common
			.log_if_verbose(&format!("Validation successful: {} rows", validated_count));
	}
...
mod tests {
	...
	async fn test_optimize_conflicting_dictionary_flags() {
		...
		assert!(result
			.unwrap_err()
			.to_string()
			.contains("Cannot specify both --dictionary and --no-dictionary"));
	}
