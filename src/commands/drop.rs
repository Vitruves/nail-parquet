...
use crate::commands::select::{
	all_columns_of_type, filter_columns_by_type, parse_row_specification,
...
#[command(after_help = "Examples:
  nail drop data.parquet -c temp_col -o cleaned.parquet
  nail drop sales.csv -r 1,5,10 -o trimmed.csv")]
pub struct DropArgs {
	#[command(flatten)]
	...
	#[arg(
		short,
		long,
		help = "Column names or regex patterns to drop (comma-separated)"
	)]
...
pub async fn execute(args: DropArgs) -> NailResult<()> {
	...
	if args.columns.is_some() || args.type_filter.is_some() {
		...
		let remaining_columns: Vec<Expr> = result_df
			.schema()
			.fields()
			.iter()
	...
	if let Some(row_spec) = &args.rows {
		...
		if is_column_condition(row_spec) {
			args.common.log_if_verbose(&format!(
				"Dropping rows based on column conditions: {}",
				row_spec
			));
			...
			args.common
				.log_if_verbose(&format!("Dropping {} rows", row_indices.len()));
