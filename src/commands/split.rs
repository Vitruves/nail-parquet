...
#[command(after_help = "Examples:
  nail split data.parquet --ratio 0.8,0.2 --names train,test --output-dir parts/
  nail split data.csv --rows 1000 --output-dir parts/")]
pub struct SplitArgs {
	...
	#[arg(
		long,
		help = "Prefix for auto-generated split file names",
		default_value = "split"
	)]
...
pub async fn execute(args: SplitArgs) -> NailResult<()> {
	...
	if ratios.len() != output_names.len() {
		return Err(NailError::InvalidArgument(format!(
			"Number of ratios ({}) must match number of names ({})",
			ratios.len(),
			output_names.len()
		)));
	}
	...
	if let Some(stratify_col) = &args.stratified_by {
		args.common.log_if_verbose(&format!(
			"Performing stratified split by column '{}' with ratios: {:?}",
			stratify_col, ratios
		));
		stratified_split(
			&df,
		...
		args.common.log_if_verbose(&format!(
			"Splitting {} rows into {} parts with ratios: {:?}",
			total_rows,
			ratios.len(),
			ratios
		));
	...
	args.common.log_if_verbose(&format!(
		"Split complete: {} files created in {}",
		output_names.len(),
		args.output_dir.display()
	));
	if args.common.verbose {
		for (i, output_name) in output_names.iter().enumerate() {
			args.common
				.log_if_verbose(&format!("  Split {}: {}", i + 1, output_name.display()));
		}
	}
...
async fn stratified_split(
			...
			if !category_array.is_null(i) && !count_array.is_null(i) {
				...
				let category = match category_array.data_type() {
					datafusion::arrow::datatypes::DataType::Utf8 => {
						if let Some(str_arr) = category_array
							.as_any()
					...
					datafusion::arrow::datatypes::DataType::Int64 => {
						if let Some(int_arr) = category_array
							.as_any()
					...
					datafusion::arrow::datatypes::DataType::Float64 => {
						if let Some(float_arr) = category_array
							.as_any()
				...
				if let Some(count_arr) = count_array
					.as_any()
					.downcast_ref::<datafusion::arrow::array::Int64Array>()
				{
	...
	if verbose {
		eprintln!(
			"Found {} categories for stratification:",
			category_counts.len()
		);
	...
	for category in category_counts.keys() {
		...
		for (i, ratio) in ratios.iter().enumerate() {
			...
			if split_size > 0 {
				let category_split = shuffled_category
					.clone()
					.limit(current_offset, Some(split_size))?;
...
fn parse_ratios(ratio_str: &str) -> NailResult<Vec<f64>> {
	...
	if (sum - 1.0).abs() < 0.001 {
		...
		Err(NailError::InvalidArgument(format!(
			"Ratios must sum to 1.0 or 100.0, got: {}",
			sum
		)))
	}
}
...
async fn shuffle_dataframe(
	df: &datafusion::prelude::DataFrame,
	jobs: Option<usize>,
) -> NailResult<datafusion::prelude::DataFrame> {
	let ctx = crate::utils::create_context_with_jobs(jobs).await?;
...
async fn shuffle_dataframe_with_seed(
	df: &datafusion::prelude::DataFrame,
	seed: u64,
	jobs: Option<usize>,
) -> NailResult<datafusion::prelude::DataFrame> {
	...
	let indices_data: Vec<_> = indices
		.iter()
		.enumerate()
	...
	let values_rows: Vec<String> = indices_data
		.iter()
	...
	if chunks.len() == 1 {
		...
		ctx.sql(&format!(
			"CREATE TEMP TABLE shuffle_map_{} (old_rn BIGINT, new_order BIGINT)",
			seed
		))
		.await?;
		...
		for chunk in chunks {
			...
			let insert_sql = format!("INSERT INTO shuffle_map_{} VALUES {}", seed, values_sql);
