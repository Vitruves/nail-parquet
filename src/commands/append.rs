...
#[command(after_help = "Examples:
  nail append base.parquet -e extra.parquet -o all.parquet
  nail append a.csv -e b.csv -o combined.csv")]
...
pub async fn execute(args: AppendArgs) -> NailResult<()> {
	...
	args.common
		.log_if_verbose(&format!("Appending {} files", append_files.len()));

	for file_path in append_files {
		let path = PathBuf::from(file_path);

		args.common
			.log_if_verbose(&format!("Appending: {}", path.display()));
	...
	args.common
		.log_if_verbose(&format!("Final dataset contains {} rows", total_rows));
...
fn schemas_compatible(
	schema1: &datafusion::common::DFSchemaRef,
	schema2: &datafusion::common::DFSchemaRef,
) -> bool {
...
async fn align_schemas(
	df: &DataFrame,
	target_schema: &datafusion::common::DFSchemaRef,
	jobs: Option<usize>,
		...
		if let Ok(resolved_name) = resolve_column_name(&current_schema, target_name) {
			...
			let null_expr = match target_field.data_type() {
				...
				_ => {
					...
					lit(
						datafusion::scalar::ScalarValue::try_from(target_field.data_type())
...
mod tests {
	...
	async fn create_test_dataframe(values: Vec<i64>, names: Vec<&str>) -> NailResult<DataFrame> {
	...
	fn test_schemas_compatible() {
		...
		let arrow_schema3 = ArrowSchema::new(vec![Field::new("id", DataType::Int64, false)]);
	...
	async fn test_align_schemas() {
		...
		let df = create_test_dataframe(vec![1, 2, 3], vec!["a", "b", "c"])
			.await
			.unwrap();
	...
	async fn test_execute_with_compatible_schemas() {
		...
		// Create base file
		let base_df = create_test_dataframe(vec![1, 2], vec!["a", "b"])
			.await
			.unwrap();
		crate::utils::io::write_data(&base_df, &base_path, None)
			.await
			.unwrap();

		// Create append file
		let append_df = create_test_dataframe(vec![3, 4], vec!["c", "d"])
			.await
			.unwrap();
		crate::utils::io::write_data(&append_df, &append_path, None)
			.await
			.unwrap();
