...
mod inspection_tests {
    ...
    fn test_head_default() {
        ...
        let output = nail().args(["head", "-i", fixtures.sample_parquet.to_str().unwrap()]).assert().success().get_output().stdout.clone();
    ...
    fn test_head_custom_number() {
        let fixtures = TestFixtures::new();
        let output = nail().args(["head", "-i", fixtures.sample_parquet.to_str().unwrap(), "-n", "2"]).assert().success().get_output().stdout.clone();
    ...
    fn test_head_to_json_file() {
        ...
        nail().args(["head", "-i", fixtures.sample_parquet.to_str().unwrap(), "-n", "3", "-o", out_file.to_str().unwrap(), "-f", "json"]).assert().success();
    ...
    fn test_tail_custom_number() {
        let fixtures = TestFixtures::new();
        nail().args(["tail", "-i", fixtures.sample_parquet.to_str().unwrap(), "-n", "2"]).assert().success().stdout(predicate::str::contains("Eve").and(predicate::str::contains("David")));
    }

    #[test]
    fn test_tail_on_small_file() {
        let fixtures = TestFixtures::new();
        let output = nail().args(["tail", "-i", fixtures.sample_parquet.to_str().unwrap(), "-n", "10"]).assert().success().get_output().stdout.clone();
    ...
    fn test_preview_reproducible() {
        let fixtures = TestFixtures::new();
        let out1 = nail().args(["preview", "-i", fixtures.sample_parquet.to_str().unwrap(), "-n", "3", "--random", "123"]).assert().success().get_output().stdout.clone();
        let out2 = nail().args(["preview", "-i", fixtures.sample_parquet.to_str().unwrap(), "-n", "3", "--random", "123"]).assert().success().get_output().stdout.clone();
    ...
    fn test_count_basic() {
        let fixtures = TestFixtures::new();
        nail().args(["count", "-i", fixtures.sample_parquet.to_str().unwrap()]).assert().success().stdout("5\n");
    }

    #[test]
    fn test_count_empty() {
        let fixtures = TestFixtures::new();
        nail().args(["count", "-i", fixtures.empty_parquet.to_str().unwrap()]).assert().success().stdout("0\n");
    }
    ...
    fn test_schema_json_output() {
        ...
        nail().args(["schema", "-i", fixtures.sample_parquet.to_str().unwrap(), "-f", "json", "-o", output_path.to_str().unwrap()]).assert().success();
    ...
    fn test_headers_basic() {
        let fixtures = TestFixtures::new();
        nail().args(["headers", "-i", fixtures.sample_parquet.to_str().unwrap()]).assert().success().stdout("id\nname\nvalue\ncategory\n");
    }

    #[test]
    fn test_headers_regex_filter() {
        let fixtures = TestFixtures::new();
        nail().args(["headers", "-i", fixtures.sample_parquet.to_str().unwrap(), "--filter", "val|cat"]).assert().success().stdout("value\ncategory\n");
    }
    
    #[test]
    fn test_size_all_options() {
        let fixtures = TestFixtures::new();
        nail().args(["size", "-i", fixtures.sample_parquet.to_str().unwrap(), "--columns", "--rows", "--bits"]).assert().success().stdout(predicate::str::contains("Per-column sizes").and(predicate::str::contains("Average bits per row")));
    }
}
...
mod manipulation_tests {
    ...
    async fn test_select_columns() {
        ...
        nail().args(["select", "-i", fixtures.sample_parquet.to_str().unwrap(), "-c", "id,name", "-o", out_cols.to_str().unwrap()]).assert().success();
    ...
    async fn test_select_rows() {
        ...
        nail().args(["select", "-i", fixtures.sample_parquet.to_str().unwrap(), "-r", "1,3-4", "-o", out_rows.to_str().unwrap()]).assert().success();
    ...
    async fn test_drop_columns() {
        ...
        nail().args(["drop", "-i", fixtures.sample_parquet.to_str().unwrap(), "-c", "value,category", "-o", out_cols.to_str().unwrap()]).assert().success();
        let df_cols = SessionContext::new().read_parquet(out_cols.to_str().unwrap(), ParquetReadOptions::default()).await.unwrap();
        assert_eq!(df_cols.schema().fields().len(), 2);
    }
    ...
    async fn test_drop_rows() {
        ...
        nail().args(["drop", "-i", fixtures.sample_parquet.to_str().unwrap(), "-r", "2,5", "-o", out_rows.to_str().unwrap()]).assert().success();
        assert_eq!(get_row_count(&out_rows).await, 3);
    }
    ...
    async fn test_filter_by_column_value() {
        ...
        nail().args(["filter", "-i", fixtures.sample_parquet.to_str().unwrap(), "-c", "id>3,category=B", "-o", out_filter.to_str().unwrap()]).assert().success();
    ...
    async fn test_filter_by_row_property() {
        ...
        nail().args(["filter", "-i", fixtures.sample_mixed_types_parquet.to_str().unwrap(), "--rows", "no-nan", "-o", out_rows.to_str().unwrap()]).assert().success();
    ...
    async fn test_search_all_options() {
        ...
        nail().args(["search", "-i", fixtures.sample_parquet.to_str().unwrap(), "--value", "li", "--ignore-case", "-c", "name", "-o", out_search.to_str().unwrap()]).assert().success();
        assert_eq!(get_row_count(&out_search).await, 2);

        let out_rows = fixtures.get_output_path("search_rows.json");
        nail().args(["search", "-i", fixtures.sample_parquet.to_str().unwrap(), "--value", "Bob", "--rows", "-f", "json", "-o", out_rows.to_str().unwrap()]).assert().success();
    ...
    async fn test_fill_value() {
        ...
        nail().args(["fill", "-i", fixtures.sample_with_nulls_parquet.to_str().unwrap(), "--method", "value", "--value", "UNKNOWN", "-c", "name", "-o", out_fill.to_str().unwrap()]).assert().success();
    ...
    async fn test_fill_mean() {
        ...
        nail().args(["fill", "-i", fixtures.sample_with_nulls_parquet.to_str().unwrap(), "--method", "mean", "-c", "value", "-o", out_mean.to_str().unwrap()]).assert().success();
...
mod transformation_tests {
    ...
    async fn test_id_creation() {
        ...
        nail().args(["id", "-i", fixtures.sample_parquet.to_str().unwrap(), "--create", "--prefix", "rec-", "--id-col-name", "record_id", "-o", output_path.to_str().unwrap()]).assert().success();
    ...
    fn test_id_error_on_existing_column() {
        let fixtures = TestFixtures::new();
        nail().args(["id", "-i", fixtures.sample_parquet.to_str().unwrap(), "--create", "--id-col-name", "name"]).assert().failure().stderr(predicate::str::contains("already exists"));
    }
    ...
    async fn test_shuffle_reproducible() {
        ...
        nail().args(["shuffle", "-i", fixtures.sample_parquet.to_str().unwrap(), "--random", "42", "-o", out1.to_str().unwrap()]).assert().success();
        nail().args(["shuffle", "-i", fixtures.sample_parquet.to_str().unwrap(), "--random", "42", "-o", out2.to_str().unwrap()]).assert().success();
    ...
    async fn test_sample_methods() {
        ...
        nail().args(["sample", "-i", fixtures.sample_parquet.to_str().unwrap(), "-n", "2", "--method", "random", "--random", "42", "-o", out_random.to_str().unwrap()]).assert().success();
        assert_eq!(get_row_count(&out_random).await, 2);

        let out_first = fixtures.get_output_path("sampled_first.parquet");
        nail().args(["sample", "-i", fixtures.sample_parquet.to_str().unwrap(), "-n", "2", "--method", "first", "-o", out_first.to_str().unwrap()]).assert().success();
        assert_eq!(get_row_count(&out_first).await, 2);

        let out_last = fixtures.get_output_path("sampled_last.parquet");
        nail().args(["sample", "-i", fixtures.sample_parquet.to_str().unwrap(), "-n", "2", "--method", "last", "-o", out_last.to_str().unwrap()]).assert().success();
    ...
    async fn test_sample_stratified() {
        ...
        nail().args(["sample", "-i", fixtures.sample_for_stratify_parquet.to_str().unwrap(), "-n", "8", "--method", "stratified", "--stratify-by", "strat_key", "-f", "json", "-o", out_strat.to_str().unwrap()]).assert().success();
    ...
    async fn test_dedup_row_wise() {
        ...
        nail().args(["dedup", "-i", fixtures.sample_with_duplicates_parquet.to_str().unwrap(), "--row-wise", "--columns", "id,name", "-o", out_dedup.to_str().unwrap()]).assert().success();
    ...
    async fn test_dedup_row_wise_keep_last() {
        ...
        nail().args(["dedup", "-i", fixtures.sample_with_duplicates_parquet.to_str().unwrap(), "--row-wise", "--keep", "last", "-o", out_last.to_str().unwrap()]).assert().success();
    ...
    async fn test_dedup_col_wise() {
        ...
        nail().args(["dedup", "-i", fixtures.sample_for_col_dedup_parquet.to_str().unwrap(), "--col-wise", "-o", out_cols.to_str().unwrap()]).assert().success();
        let df_cols = SessionContext::new().read_parquet(out_cols.to_str().unwrap(), ParquetReadOptions::default()).await.unwrap();
        assert_eq!(df_cols.schema().fields().len(), 2);
    }
}
...
mod combination_tests {
    ...
    async fn test_merge_inner_and_left() {
        ...
        nail().args(["merge", "-i", fixtures.sample_parquet.to_str().unwrap(), "--right", fixtures.sample2_parquet.to_str().unwrap(), "--key-mapping", "id=user_id", "-o", out_inner.to_str().unwrap()]).assert().success();
        assert_eq!(get_row_count(&out_inner).await, 2);

        let out_left = fixtures.get_output_path("merged_left.parquet");
        nail().args(["merge", "-i", fixtures.sample_parquet.to_str().unwrap(), "--right", fixtures.sample2_parquet.to_str().unwrap(), "--key-mapping", "id=user_id", "--left-join", "-o", out_left.to_str().unwrap()]).assert().success();
    ...
    async fn test_merge_right_join() {
        ...
        nail().args(["merge", "-i", fixtures.sample_parquet.to_str().unwrap(), "--right", fixtures.sample2_parquet.to_str().unwrap(), "--key-mapping", "id=user_id", "--right-join", "-o", out_right.to_str().unwrap()]).assert().success();
    ...
    async fn test_append_ignore_schema() {
        ...
        nail().args(["append", "-i", fixtures.sample_parquet.to_str().unwrap(), "--files", fixtures.sample_csv.to_str().unwrap(), "--ignore-schema", "-o", out_append.to_str().unwrap()]).assert().success();
    ...
    async fn test_split_by_ratio_and_names() {
        ...
        nail().args(["split", "-i", fixtures.sample_parquet.to_str().unwrap(), "--ratio", "0.6,0.4", "--names", "train,test", "--output-dir", output_dir.to_str().unwrap(), "--random", "123"]).assert().success();
    ...
    async fn test_split_stratified() {
        let fixtures = TestFixtures::new();
        let output_dir = &fixtures.output_dir;
        nail().args(["split", "-i", fixtures.sample_for_stratify_parquet.to_str().unwrap(), "--ratio", "0.5,0.5", "--stratified-by", "strat_key", "--output-dir", output_dir.to_str().unwrap()]).assert().success();
...
mod format_and_analysis_tests {
    ...
    async fn test_convert_round_trip() {
        ...
        nail().args(["convert", "-i", fixtures.sample_parquet.to_str().unwrap(), "-o", csv_output.to_str().unwrap()]).assert().success();
        let parquet_output = fixtures.get_output_path("roundtrip.parquet");
        nail().args(["convert", "-i", csv_output.to_str().unwrap(), "-o", parquet_output.to_str().unwrap()]).assert().success();
    ...
    fn test_stats_all_types() {
        ...
        nail().args(["stats", "-i", fixtures.sample_parquet.to_str().unwrap(), "-t", "basic", "-f", "json", "-o", out_basic.to_str().unwrap()]).assert().success();
        ...
        nail().args(["stats", "-i", fixtures.sample_parquet.to_str().unwrap(), "-t", "exhaustive", "-f", "json", "-o", out_ex.to_str().unwrap()]).assert().success();
    ...
    fn test_correlations_all_options() {
        ...
        nail().args(["correlations", "-i", fixtures.sample_parquet.to_str().unwrap(), "-c", "id,value", "-f", "json", "-o", out_corr.to_str().unwrap()]).assert().success();
        ...
        nail().args(["correlations", "-i", fixtures.sample_parquet.to_str().unwrap(), "-c", "id,value", "--correlation-matrix", "--digits", "2", "-f", "json", "-o", out_matrix.to_str().unwrap()]).assert().success();
...
mod error_and_edge_case_tests {
    ...
    fn test_file_not_found() {
        nail().args(["head", "-i", "nonexistent.parquet"]).assert().failure().stderr(predicate::str::contains("No such file"));
    }

    #[test]
    fn test_invalid_column() {
        let fixtures = TestFixtures::new();
        nail().args(["select", "-i", fixtures.sample_parquet.to_str().unwrap(), "-c", "bad_col"]).assert().failure().stderr(predicate::str::contains("Columns not found"));
    }

    #[test]
    fn test_merge_missing_key() {
        let fixtures = TestFixtures::new();
        nail().args(["merge", "-i", fixtures.sample_parquet.to_str().unwrap(), "--right", fixtures.sample2_parquet.to_str().unwrap()]).assert().failure().stderr(predicate::str::contains("must be specified"));
    }

    #[test]
    fn test_split_invalid_ratio() {
        let fixtures = TestFixtures::new();
        nail().args(["split", "-i", fixtures.sample_parquet.to_str().unwrap(), "--ratio", "0.5,0.6"]).assert().failure().stderr(predicate::str::contains("must sum to 1.0 or 100.0"));
    }
    
    #[test]
    fn test_correlation_non_numeric() {
        let fixtures = TestFixtures::new();
        nail().args(["correlations", "-i", fixtures.sample_parquet.to_str().unwrap(), "-c", "name"]).assert().failure().stderr(predicate::str::contains("Correlation requires numeric columns only"));
    }

    #[test]
    fn test_append_schema_mismatch() {
        let fixtures = TestFixtures::new();
        nail().args(["append", "-i", fixtures.sample_parquet.to_str().unwrap(), "--files", fixtures.sample2_parquet.to_str().unwrap()]).assert().failure().stderr(predicate::str::contains("Schema mismatch"));
    }

    #[test]
    fn test_dedup_missing_mode() {
        let fixtures = TestFixtures::new();
        nail().args(["dedup", "-i", fixtures.sample_parquet.to_str().unwrap()]).assert().failure().stderr(predicate::str::contains("Must specify either --row-wise or --col-wise"));
    }
    ...
    fn test_empty_file_operations() {
        ...
        nail().args(["head", "-i", empty]).assert().success();
        nail().args(["stats", "-i", empty]).assert().success();
        nail().args(["count", "-i", empty]).assert().success().stdout("0\n");
    }
}