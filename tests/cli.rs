...
mod inspection_tests {
	...
	fn test_head_default() {
		...
		let output = nail()
			.args(["head", fixtures.sample_parquet.to_str().unwrap()])
			.assert()
			.success()
			.get_output()
			.stdout
			.clone();
		assert_eq!(
			String::from_utf8(output).unwrap().matches("Record").count(),
			5
		);
	}
	...
	fn test_head_custom_number() {
		let fixtures = TestFixtures::new();
		let output = nail()
			.args(["head", fixtures.sample_parquet.to_str().unwrap(), "-n", "2"])
			.assert()
			.success()
			.get_output()
			.stdout
	...
	fn test_tail_custom_number() {
		let fixtures = TestFixtures::new();
		nail()
			.args(["tail", fixtures.sample_parquet.to_str().unwrap(), "-n", "2"])
			.assert()
			.success()
			.stdout(predicate::str::contains("Eve").and(predicate::str::contains("David")));
	}
	...
	fn test_tail_on_small_file() {
		...
		assert_eq!(
			String::from_utf8(output).unwrap().matches("Record").count(),
			5,
			"Should return all rows if n > row_count"
		);
	}
	...
	fn test_count_empty() {
		let fixtures = TestFixtures::new();
		nail()
			.args(["count", fixtures.empty_parquet.to_str().unwrap()])
			.assert()
			.success()
			.stdout("0\n");
	}
	...
	fn test_headers_basic() {
		let fixtures = TestFixtures::new();
		nail()
			.args(["headers", fixtures.sample_parquet.to_str().unwrap()])
			.assert()
			.success()
			.stdout("id\nname\nvalue\ncategory\n");
	}
	...
	fn test_size_all_options() {
		...
		nail()
			...
			.success()
			.stdout(
				predicate::str::contains("Per-column sizes")
					.and(predicate::str::contains("Average bits per row")),
			);
	}
}
...
mod manipulation_tests {
	...
	async fn test_select_columns() {
		...
		nail()
			.args([
				...
				"id,name",
				"-o",
				out_cols.to_str().unwrap(),
			])
			.assert()
			.success();
		let df_cols = SessionContext::new()
			.read_parquet(out_cols.to_str().unwrap(), ParquetReadOptions::default())
			.await
			.unwrap();
	...
	async fn test_filter_by_row_property() {
		...
		nail()
			.args([
				"filter",
				fixtures.sample_mixed_types_parquet.to_str().unwrap(),
				"--rows",
				"no-nan",
				"-o",
				out_rows.to_str().unwrap(),
			...
			.success();
		assert_eq!(
			get_row_count(&out_rows).await,
			2,
			"Two rows should have no NaN values"
		);
	}
	...
	async fn test_search_all_options() {
		...
		assert!(
			row_data["row_number"].is_number(),
			"row_number should be a number"
		);
		let row_num = row_data["row_number"].as_u64().unwrap();
		assert!(
			row_num == 1 || row_num == 2,
			"Bob should be in row 1 (0-based) or 2 (1-based)"
		);
	}
	...
	async fn test_fill_value() {
		...
		nail()
			.args([
				"fill",
				fixtures.sample_with_nulls_parquet.to_str().unwrap(),
				"--method",
				"value",
				"--value",
				"UNKNOWN",
				...
				"name",
				"-o",
				out_fill.to_str().unwrap(),
			])
			.assert()
			.success();
		let df_fill = SessionContext::new()
			.read_parquet(out_fill.to_str().unwrap(), ParquetReadOptions::default())
			.await
			.unwrap();
		assert_eq!(
			df_fill
				.filter(datafusion::prelude::col("name").is_null())
				.unwrap()
				.count()
				.await
				.unwrap(),
	...
	async fn test_fill_mean() {
		...
		nail()
			.args([
				...
				"value",
				"-o",
				out_mean.to_str().unwrap(),
			])
			.assert()
			.success();
		let df_mean = SessionContext::new()
			.read_parquet(out_mean.to_str().unwrap(), ParquetReadOptions::default())
			.await
			.unwrap();
		assert_eq!(
			df_mean
				.filter(datafusion::prelude::col("value").is_null())
				.unwrap()
				.count()
				.await
				.unwrap(),
...
mod transformation_tests {
	...
	async fn test_sample_stratified() {
		...
		nail()
			.args([
				"sample",
				fixtures.sample_for_stratify_parquet.to_str().unwrap(),
				"-n",
				"8",
				"--method",
				"stratified",
	...
	async fn test_dedup_row_wise() {
		...
		nail()
			.args([
				"dedup",
				fixtures.sample_with_duplicates_parquet.to_str().unwrap(),
				"--row-wise",
				"--columns",
				"id,name",
				"-o",
				out_dedup.to_str().unwrap(),
			])
			.assert()
			.success();
	...
	async fn test_dedup_col_wise() {
		...
		nail()
			.args([
				"dedup",
				fixtures.sample_for_col_dedup_parquet.to_str().unwrap(),
				"--col-wise",
				"-o",
				out_cols.to_str().unwrap(),
			])
...
mod combination_tests {
	...
	async fn test_merge_inner_and_left() {
		...
		nail()
			.args([
				...
				fixtures.sample_parquet.to_str().unwrap(),
				"--right",
				fixtures.sample2_parquet.to_str().unwrap(),
				"--key-mapping",
				"id=user_id",
				"-o",
	...
	async fn test_append_ignore_schema() {
		...
		nail()
			.args([
				...
				fixtures.sample_parquet.to_str().unwrap(),
				"--files",
				fixtures.sample_csv.to_str().unwrap(),
				"--ignore-schema",
				"-o",
				out_append.to_str().unwrap(),
			])
			.assert()
			.success();
		...
		assert_eq!(
			content.lines().count(),
			9,
			"Should be 1 header + 8 data rows (5+3)"
		);
	}
	...
	async fn test_split_stratified() {
		...
		assert!(
			(9..=11).contains(&count1),
			"Split 1 should have roughly half the rows (9-11)"
		);
		assert!(
			(9..=11).contains(&count2),
			"Split 2 should have roughly half the rows (9-11)"
		);
	}
}
...
mod format_and_analysis_tests {
	...
	fn test_stats_all_types() {
		...
		let numeric_line = content_basic
			.trim()
			.lines()
	...
	fn test_correlations_all_options() {
		...
		if diff > 0.02 {
			...
			eprintln!(
				"Available keys: {:?}",
				first_row.as_object().unwrap().keys().collect::<Vec<_>>()
			);
		}

		assert!(
			diff <= 0.02,
			"corr_with_id ({}) is not close enough to 1.0 (diff: {})",
			corr_with_id,
			diff
		);
	}
...
mod error_and_edge_case_tests {
	...
	fn test_split_invalid_ratio() {
		...
		nail()
			...
			.assert()
			.failure()
			.stderr(predicate::str::contains("must sum to 1.0 or 100.0"));
	}
	...
	fn test_correlation_non_numeric() {
		...
		nail()
			...
			.stderr(predicate::str::contains(
				"Correlation requires numeric columns only",
			));
	}
	...
	fn test_dedup_missing_mode() {
		let fixtures = TestFixtures::new();
		nail()
			.args(["dedup", fixtures.sample_parquet.to_str().unwrap()])
			.assert()
			.failure()
			.stderr(predicate::str::contains(
				"Must specify either --row-wise or --col-wise",
			));
	}
...
mod new_command_tests {
	...
	fn test_optimize_default_output_name() {
		...
		nail()
			.args(["optimize", input_copy.to_str().unwrap()])
			.assert()
			.success();
	...
	fn test_optimize_conflicting_dictionary_options() {
		...
		nail()
			...
			.failure()
			.stderr(predicate::str::contains(
				"Cannot specify both --dictionary and --no-dictionary",
			));
	}
		...
		nail()
			...
			.stdout(
				predicate::str::contains("Low")
					.or(predicate::str::contains("Medium"))
					.or(predicate::str::contains("High")),
			);
	}
	...
	fn test_binning_label_count_mismatch() {
		...
		nail()
			.args([
				...
				"3",
				"--labels",
				"Low,High", // Only 2 labels for 3 bins
			])
			.assert()
			.failure()
			.stderr(predicate::str::contains(
				"Number of labels (2) must match number of bins (3)",
			));
	}
	...
	fn test_pivot_multiple_columns_works() {
		...
		nail()
			.args([
				...
				"category",
				"-c",
				"name,id", // Multiple pivot columns
				"-l",
				"value",
			])
			.assert()
	...
	fn test_pivot_multiple_values_works() {
		...
		nail()
			.args([
				...
				"name",
				"-l",
				"value,id", // Multiple value columns
			])
			.assert()
			.success();
	}
	...
	fn test_pivot_invalid_value_column() {
		...
		nail()
			.args([
				...
				"name",
				"-l",
				"name", // String column as value
			])
			.assert()
			.failure()
			.stderr(predicate::str::contains(
				"Value column 'name' must be numeric",
			));
	}
	...
	fn test_pivot_auto_detect_value_columns() {
		...
		nail()
			.args([
				...
				"name",
				"-c",
				"category", // No value columns specified, should auto-detect
			])
			.assert()
			.success();
	}
