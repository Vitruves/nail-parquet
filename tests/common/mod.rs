...
fn create_sample_for_binning_parquet(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
	...
	let batch = RecordBatch::try_new(
		...
		vec![
			...
			Arc::new(Float64Array::from(vec![
				15.5, 25.0, 35.7, 45.2, 55.9, 65.1, 75.4, 85.8, 95.3, 105.6,
			])),
			Arc::new(Int64Array::from(vec![
				18, 22, 25, 28, 32, 35, 40, 45, 50, 55,
			])),
			Arc::new(StringArray::from(vec![
				"A", "B", "A", "C", "B", "A", "C", "B", "A", "C",
			])),
...
fn create_sample_for_pivot_parquet(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
	...
	let batch = RecordBatch::try_new(
		...
		vec![
			...
			Arc::new(Float64Array::from(vec![
				1000.0, 1500.0, 800.0, 1200.0, 900.0, 1100.0, 700.0, 1000.0, 600.0, 750.0, 850.0,
				950.0,
			])),
			Arc::new(Int64Array::from(vec![
				10, 15, 8, 12, 9, 11, 7, 10, 6, 7, 8, 9,
			])),
			Arc::new(StringArray::from(vec![
